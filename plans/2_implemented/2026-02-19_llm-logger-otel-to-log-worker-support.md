# Plan: LLM Telemetry via Log Worker (llemtry)
...
```
llm-logger plugin ──POST /llemtry──→ Log Worker ──→ Langfuse API
...
### Part 1: Log Worker — new `/llemtry` endpoint

**Files to modify/create:**

| File | Action |
|------|--------|
| `workers/log-receiver/src/index.ts` | Add `/llemtry` route |
| `workers/log-receiver/src/llemtry.ts` | NEW — handle llemtry batches, dispatch to backends |
| `workers/log-receiver/src/backends/langfuse.ts` | NEW — convert llemtry spans to Langfuse ingestion format |
...
```typescript
// POST /llemtry — accepts a batch of LLM generation spans
interface LlemtryBatch {
  resource: {
...
- Parse and validate the `LlemtryBatch`
...
Converts llemtry spans to Langfuse batch ingestion format:

| Llemtry field | Langfuse field |
|--------------|----------------|
...
const LLEMTRY_LOGGING = process.env.ENABLE_LLEMTRY_LOGGING === 'true'
const LLEMTRY_URL = process.env.LOG_WORKER_URL?.replace(/\/logs$/, '/llemtry')
...
- **Llemtry logging**: Only when `ENABLE_LLEMTRY_LOGGING=true`. Sends to Log Worker `/llemtry` endpoint. Derives URL from existing `LOG_WORKER_URL` (replaces `/logs` suffix with `/llemtry`). Uses existing `LOG_WORKER_TOKEN` for auth.
...
Note: `LOG_WORKER_URL` and `LOG_WORKER_TOKEN` are already available if Vector log shipping is enabled. The plugin derives the llemtry URL from these. No new URL/token config needed.
...
```markdown
## 7.x LLM Telemetry (llemtry)

If `ENABLE_LLEMTRY_LOGGING=true` in `openclaw-config.env`:

1. **Log Worker reachable**: `curl -s https://<LOG_WORKER_URL>/health` returns `{"status":"ok"}`
2. **Llemtry endpoint exists**: `curl -s -X POST https://<LOG_WORKER_URL_BASE>/llemtry -H "Authorization: Bearer <LOG_WORKER_TOKEN>" -H "Content-Type: application/json" -d '{"resource":{"serviceName":"test"},"spans":[]}' ` returns `{"status":"ok","count":0}`
...
**Critical validation in llm-logger startup**: When `ENABLE_LLEMTRY_LOGGING=true`, the plugin should validate on registration that:

- `LOG_WORKER_URL` is set and non-empty
- `LOG_WORKER_TOKEN` is set and non-empty
- Log a clear warning (not silent failure) if either is missing, and disable llemtry output
...
| File | Action | Lines |
|------|--------|-------|
| `workers/log-receiver/src/index.ts` | Add `/llemtry` route (4 lines) | ~4 |
| `workers/log-receiver/src/llemtry.ts` | NEW — parse batch, log summary, dispatch to backends | ~60 |
...
| `deploy/plugins/llm-logger/index.js` | Add llemtry output mode (span assembly, HTTP send, validation) | ~80 |
| `deploy/docker-compose.override.yml` | Add `ENABLE_LLEMTRY_LOGGING`, `OPENCLAW_INSTANCE_ID`, `VPS_HOSTNAME` env vars | ~3 |
| `openclaw-config.env.example` | Document `OPENCLAW_INSTANCE_ID`, `ENABLE_LLEMTRY_LOGGING` | ~2 |
| `playbooks/07-verification.md` | Add llemtry verification section | ~15 |

## Verification (end-to-end)

1. **Deploy Log Worker**: `cd workers/log-receiver && wrangler deploy` + set Langfuse secrets
2. **Test endpoint**: `curl -X POST .../llemtry` with empty batch → `{"status":"ok","count":0}`
3. **Enable on gateway**: Set `ENABLE_LLEMTRY_LOGGING=true` in docker-compose env, enable llm-logger plugin, restart gateway
...
6. **Check Log Worker**: `wrangler tail` or CF dashboard — should see llemtry summary entries
7. **Check Langfuse**: Traces appear with `sessionId`, `agentId`, model, token usage
8. **Validation test**: Set `ENABLE_LLEMTRY_LOGGING=true` without `LOG_WORKER_URL` → should see error log, no crashes
