...
- **Use the `openclaw` CLI wrapper for OpenClaw commands.** VPS host: `openclaw <subcommand>` (wrapper at `/usr/local/bin/openclaw`). Inside container: `openclaw <subcommand>` (symlink). For explicit docker exec, always use `--user node`: `sudo docker exec --user node openclaw-main-claw openclaw <subcommand>`.
