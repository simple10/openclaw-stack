...
if ! $DRY_RUN; then
  ...
  # Track claws with no local workspace (for sync-down hint in summary).
  # Source config + instances lib (config was generated by step 1's pre-deploy).
  source "$SCRIPT_DIR/lib/source-config.sh" 2>/dev/null || true
  source "$SCRIPT_DIR/lib/instances.sh"
  resolve_instance_list "${INSTANCE:-all}"
  ws_check_list="$INSTANCE_LIST"
  for claw in $ws_check_list; do
    if [ ! -d "$REPO_ROOT/openclaw/${claw}/workspace" ]; then
      MISSING_WORKSPACES+=("$claw")
    fi
  done
...
if $DRY_RUN; then
  ...
  # Source SSH config (source-config.sh already sourced in step 3)
...
if $DRY_RUN; then
  ...
  if [ ${#MISSING_WORKSPACES[@]} -gt 0 ]; then
    echo ""
    info "Optional: sync workspace files from VPS for: ${MISSING_WORKSPACES[*]}"
    info "  scripts/sync-workspaces.sh down          # markdown files only"
    info "  scripts/sync-workspaces.sh down --all    # all files"
  fi
fi
