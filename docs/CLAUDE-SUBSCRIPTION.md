...
```bash
# 1. Save the claude auth token via the config UI
#    Visit: https://<AI_GATEWAY_WORKER_URL>/config
#    Authenticate with your gateway token
#    Paste the sk-ant-oat-* token into the "Anthropic OAuth Token" field
#    Save

# 2. Configure openclaw to act like claude code...
#    Copy AI_GATEWAY_TOKEN from .env with sk-ant-oat- prefixed
(source .env && printf "\nsk-ant-oat-$AI_GATEWAY_TOKEN\n\n")

# SSH into the VPS
./scripts/ssh-vps.sh

openclaw models auth paste-token --provider anthropic
# Paste in the sk-ant-oat-XXXX token
...
# To rotate your setup token:
# 1. Rerun `claude setup-token`
# 2. Update the token via the config UI at /config
```
