...
const AUTO_GENERATED_HEADER =
  '# ── Auto-generated (managed by pre-deploy — do not edit above this line) ──'
...
async function queryVpsCapacity(env) {
  ...
  if (exitCode !== 0) {
    fatal(
      `SSH to VPS failed (exit ${exitCode}): ${stderr.trim()}\nCannot resolve resource percentages.`
    )
  }
...
function generateStackEnv(env, config, claws) {
  ...
  if (stack.egress_proxy) {
    lines.push(
      `STACK__STACK__EGRESS_PROXY__AUTH_TOKEN=${formatEnvValue(stack.egress_proxy.auth_token)}`
    )
  }
  lines.push('')
  ...
  for (const [name, claw] of Object.entries(claws)) {
    ...
    lines.push(
      `STACK__CLAWS__${envKey}__DASHBOARD_PATH=${formatEnvValue(claw.dashboard_path || '')}`
    )
...
async function main() {
  ...
  if (!config.claws || Object.keys(config.claws).length === 0)
    fatal('stack.yml has no claws defined')
  ...
  if (stack.sandbox_registry_container) {
    ...
    if (result.status === 0) {
      ...
      result = spawnSync(
        'docker',
        ['run', '--rm', 'httpd:2', 'htpasswd', '-nbB', 'openclaw', token],
        { encoding: 'utf8' }
      )
      if (result.status === 0) {
        htpasswdLine = result.stdout.trim()
