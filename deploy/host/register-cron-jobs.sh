...
# Runs daily at 4 AM (after backup/prune, before daily report).
...
if [ "${STACK__STACK__OPENCLAW__AUTO_UPDATE:-false}" = "true" ]; then
  {
    ...
    echo ""
    echo "0 4 * * * openclaw ${INSTALL_DIR}/host/auto-update-openclaw.sh >> ${INSTALL_DIR}/logs/auto-update.log 2>&1"
  } > /etc/cron.d/openclaw-auto-update
  chmod 644 /etc/cron.d/openclaw-auto-update
  echo "  Installed /etc/cron.d/openclaw-auto-update (daily at 4 AM)"
else
